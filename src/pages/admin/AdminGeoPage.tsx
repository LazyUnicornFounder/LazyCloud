...
export default function AdminGeoPage() {
  ...
  return (
    <div>
      ...
      <div className="mt-8 border border-[#f0ead6]/8 p-5 flex flex-col sm:flex-row sm:items-center gap-4">
        <div className="flex items-center gap-4 flex-1">
          ...
          <div>
            ...
            <p className="font-body text-[13px] text-[#f0ead6]/72">citation rate across {citations.length} tests</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <div className="text-right">
            <p className="font-body text-[13px] text-[#f0ead6]/68">{pendingQueries.length} uncovered queries</p>
            <p className="font-body text-[13px] text-[#f0ead6]/68">{coveredQueries.length} with content</p>
          </div>
          <button
            onClick={runCitationTest}
            disabled={testingCitation}
            className="inline-flex items-center gap-2 border border-[#f0ead6]/10 px-4 py-2 font-body text-xs text-[#f0ead6]/88 hover:text-[#f0ead6] transition-colors disabled:opacity-40"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-px bg-[#f0ead6]/5 mt-6">
        ...
        <div className="bg-[#0f0f0d] p-5 lg:col-span-1">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <p className="font-body text-[13px] tracking-[0.15em] uppercase text-[#f0ead6]/82">Target Queries</p>
            </div>
            <span className="font-body text-[13px] text-[#f0ead6]/68">{queries.length} total</span>
          </div>
          <input
            type="text"
            value={qSearch}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      {q.product && q.product !== 'general' && (
                        <span className="font-body text-[13px] px-1.5 py-0.5 border border-[#c8a961]/20 text-[#c8a961]/60 uppercase tracking-wider">{q.product}</span>
                      )}
                      {q.query_type && (
                        <span className={`font-body text-[13px] px-1.5 py-0.5 border ${typeColors[q.query_type] || "text-[#f0ead6]/75 border-[#f0ead6]/10"} uppercase tracking-wider`}>{q.query_type}</span>
                      )}
                      <span className="font-body text-[13px] text-[#f0ead6]/68">P{q.priority}</span>
                    </div>
                  </div>
                  <div className="shrink-0 ml-2">
                    {q.has_content ? (
                      <span className="font-body text-[13px] text-emerald-400/50 border border-emerald-400/20 px-1.5 py-0.5">covered</span>
                    ) : q.brand_cited ? (
                      <span className="font-body text-[13px] text-[#c8a961]/50 border border-[#c8a961]/20 px-1.5 py-0.5">cited</span>
                    ) : (
                      <span className="font-body text-[13px] text-red-400/30 border border-red-400/10 px-1.5 py-0.5">pending</span>
                    )}
                  </div>
                </div>
              ))
        ...
        <div className="bg-[#0f0f0d] p-5 lg:col-span-1">
          <div className="flex items-center gap-2 mb-4">
            ...
            <p className="font-body text-[13px] tracking-[0.15em] uppercase text-[#f0ead6]/82">Published GEO Posts</p>
                ...
                <div key={post.id} className="flex items-start justify-between py-2.5 px-2 border-b border-[#f0ead6]/5 hover:bg-[#f0ead6]/3 transition-colors">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      {post.target_query && (
                        <span className="font-body text-[13px] px-1.5 py-0.5 border border-emerald-400/20 text-emerald-400/50 truncate max-w-[120px]">{post.target_query}</span>
                      )}
              ...
              queuedDrafts.map((draft: any, i: number) => (
                <div key={draft.id} className="flex items-start gap-3 py-2.5 px-2 border-b border-[#f0ead6]/5 hover:bg-[#f0ead6]/3 transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <span className="font-body text-[13px] px-1.5 py-0.5 border border-emerald-400/20 text-emerald-400/50">GEO</span>
      ...
      <div className="mt-8">
        <p className="font-body text-[13px] tracking-[0.15em] uppercase text-[#f0ead6]/75 mb-3">Recent Citation Tests</p>
        <div className="border border-[#f0ead6]/8 overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr className="border-b border-[#f0ead6]/8">
                <th className="text-left px-4 py-2.5 font-body text-[13px] tracking-[0.12em] uppercase text-[#f0ead6]/72 font-normal">Query</th>
                <th className="text-left px-4 py-2.5 font-body text-[13px] tracking-[0.12em] uppercase text-[#f0ead6]/72 font-normal">Cited</th>
                <th className="text-left px-4 py-2.5 font-body text-[13px] tracking-[0.12em] uppercase text-[#f0ead6]/72 font-normal">Confidence</th>
                <th className="text-left px-4 py-2.5 font-body text-[13px] tracking-[0.12em] uppercase text-[#f0ead6]/72 font-normal">Tested</th>
              </tr>
            </thead>
              ...
              {citations.length === 0 ? (
                ...
                citations.map((c: any) => (
                  <tr key={c.id} className="hover:bg-[#f0ead6]/3 transition-colors">
                    ...
                    <td className="px-4 py-2.5 font-body text-xs text-[#f0ead6]/82">{c.confidence ?? "—"}</td>
                    <td className="px-4 py-2.5 font-body text-xs text-[#f0ead6]/75">{new Date(c.tested_at).toLocaleDateString()}</td>
                  </tr>
                ))
              )}
            </tbody>
