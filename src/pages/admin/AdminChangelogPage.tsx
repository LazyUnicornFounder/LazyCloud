...
const EMPTY: Partial<Release> = {
  agents?_name: "", version: "", release_date: new Date().toISOString().split("T")[0],
...
export default function AdminChangelogPage() {
  ...
  const handleSave = async () => {
    if (!editing?.agents?_name || !editing?.version || !editing?.summary) {
          ...
          <tbody>
            {releases.map(r => (
              <tr key={r.id} className="border-b border-[#f0ead6]/15 last:border-0">
                <td className="px-3 py-2 font-body text-[14px] text-[#f0ead6]/92">{r.agents?_name}</td>
