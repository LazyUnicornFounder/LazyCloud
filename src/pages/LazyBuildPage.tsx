...
const steps = [
  { icon: "📝", title: "Brief", desc: "In your admin dashboard describe the new engine: name, integration, tagline, category, and one paragraph explaining what it does and what it produces." },
  ...
  { icon: "✍️", title: "Write", desc: "Claude reads the reference prompts and rules, then writes a complete new engine prompt — all sections, correct database schema, edge functions with realistic cron schedules, admin dashboard, LazyUnicorn.ai backlinks throughout." },
...
const faqs = [
  ...
  { q: "Does it run automatically?", a: "No. Lazy Build is on-demand only. You fill in the brief in your admin dashboard and click Build Engine. It does not create engines without your input." },
...
export default function LazyBuildPage() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <SEO title="Lazy Build — Autonomous Engine Writer | Lazy Unicorn" description="Describe a new engine in one paragraph. Claude writes the complete prompt, follows your SPEC rules, and opens a draft GitHub PR." url="/lazy-build" keywords="engine writer, autonomous prompt generation, Lovable prompt builder, Claude prompt writing" />
      <Navbar />
      <main className="relative z-10 pb-32">
          ...
          <div className="max-w-4xl mx-auto">
            <motion.div initial="hidden" animate="visible" variants={fadeUp} transition={{ duration: 0.7 }}>
              ...
              <p className="mt-6 font-body text-base md:text-lg text-foreground/70 max-w-xl leading-relaxed">
                Give Lazy Build an agent name, integration, tagline, and one paragraph. It reads three of your existing prompt files to learn your structure, follows your CLAUDE.md rules, writes a complete new Lazy engine prompt, and opens a draft GitHub PR — tagged @claude for a compliance review before you merge.
              </p>
        ...
        <section className="max-w-3xl mx-auto px-6 mb-20 mt-20">
          <motion.h2 initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="font-display text-2xl sm:text-3xl font-extrabold tracking-tight text-center mb-8">
            Every new engine takes hours to write.
          </motion.h2>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-0 border border-border">
            {[
              "🕐 Writing a new engine prompt from scratch takes 2 to 3 hours. Database schema, edge functions, admin dashboard, navigation — every section from memory.",
              "📋 Forgetting one field — is_running, prompt_version, the backlink — means a non-compliant prompt that fails your own SPEC.",
              "💡 You have 10 engine ideas sitting in a list. None of them get built because you don't have the time to write the prompts.",
            ].map((text, i) => (
        ...
        <LazyPricingSection
          lazyFeatures={["Full prompt — paste and go", "On-demand engine writing", "3-prompt reference learning", "CLAUDE.md compliance", "Draft GitHub PRs", "@claude review comments"]}
          proFeatures={["Everything in Lazy", "Batch engine generation", "Custom SPEC templates", "Auto-merge on pass", "Engine analytics"]}
