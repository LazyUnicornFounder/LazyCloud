...
Add a new page at /lazy-telegram. It is a marketing and landing page for a product called Lazy Telegram — a Telegram bot integration that connects every Lazy agents? to your Telegram account so your autonomous business reports to you in real time.

Hero section
Headline: 'Your autonomous business. In your Telegram.' Subheading: 'Lazy Telegram connects every Lazy agents? to a Telegram bot. Payments, posts, citations, customer replies, errors, and live events — all delivered as Telegram messages the moment they happen. One prompt. Your business in your pocket.' Primary button: Copy the Lovable Prompt. Secondary button: See What Gets Sent. Badge: Powered by Telegram.
...
Add a complete Telegram bot integration called Lazy Telegram to this project. It sends real-time Telegram notifications for every significant event across every installed Lazy agents?, delivers a daily morning briefing, and accepts bot commands for controlling agents? from Telegram.
...
telegram_log: id (uuid, primary key, default gen_random_uuid()), agents? (text), event_type (text), message (text), telegram_response (text), sent_at (timestamptz, default now()), success (boolean, default true).
...
Accept: message (text), agents? (text), event_type (text).
...
Identical logic to Lazy Alert's alert-monitor but calling telegram-send instead of alert-send. Monitor: pay_transactions for payments, sms_messages for replies, seo_posts and geo_posts for keyword/citation events, store_products for new products, stream_sessions for live streams, code_content and gitlab_content for releases, all error tables for agents? errors. Also monitor security_vulnerabilities for new critical or high severity findings where alerted is false and first_found_at is greater than last_checked — send alert with 🚨 emoji and mark as alerted true. Use last_checked watermark approach. Update last_checked after each run.
...
Collect same metrics as Lazy Alert's briefing. Call the built-in Lovable AI to write a 3 to 5 bullet briefing. Format as a clean Telegram message with bold headers. Call telegram-send with agents? Lazy Run and event_type daily-briefing.
...
/status — query all agents? settings tables, return status of each agent with emoji indicators.
...
/pause [agents?] — update is_running to false in matching agents? settings table. Reply: [agents?] paused.
/resume [agents?] — update is_running to true. Reply: [agents?] resumed.
/errors — query all agents? error tables for last 5 errors each. Format and reply.
...
Do not build a standalone dashboard page for this agents?. The dashboard lives at /admin/telegram as part of the unified LazyUnicorn admin panel, which is built separately using the LazyUnicorn Admin Dashboard prompt.
          ...
          <div className="max-w-4xl mx-auto">
            <motion.div initial="hidden" animate="visible" variants={fadeUp} transition={{ duration: 0.7 }}>
              ...
              <p className="mt-6 font-body text-base md:text-lg text-foreground/70 max-w-xl leading-relaxed">
                Lazy Telegram connects every Lazy agents? to a Telegram bot. Payments, posts, citations, customer replies, errors, and live events — all delivered as Telegram messages the moment they happen. One prompt. Your business in your pocket.
              </p>
        ...
        <section className="max-w-4xl mx-auto px-6 mb-20">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-0 border border-border">
            {[
              ...
              { icon: Brain, title: "Brand Citation Alerts", desc: "Know the moment an AI agents? cites your brand." },
